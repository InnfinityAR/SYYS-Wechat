...
/*----------------------------注释结束--程序开始-----------------------------------------------------------*/
var useHtml5=true,HTML5Effects=true,Shear,cropwidth=parent.cropwidth,defaultImg=parent.defaultImg, //默认背景图片
	deleteImgUrl=parent.deleteImgUrl, //删除图片
	uploadImgUrl=parent.uploadImgUrl,//上传图片
	cropImgUrl=parent.cropImgUrl,
	relativeUrl=parent.relativeUrl,
	styleUrl=parent.relativeUrl,
	saveUrl=parent.saveUrl;
function setCropConfig(HTML5,HTML5Effects){
        HTML5 = HTML5 || false;
        HTML5Effects = HTML5Effects || true;
	useHtml5=HTML5;
	HTML5Effects=HTML5Effects;
	document.getElementById('saveWidth').value=cropwidth;
}
window.ShearPhoto.MINGGE(function() {
	...
	Shear.config({
		...
		/*HTML5重点功能*/
		translate3d: true, //是否开启3D移动，CPU加速。可选true  false。默认关闭的，作者认为PC端没必要！在PC端开启后，有部份浏览器页面走位的问题。主要是各大浏览器不统一所致，手机端效果会明显！PC端没什么感觉。 原来是采用left top进行定位的，那么3D移动就是CSS3的translate3d属性。去百度一下translate3D吧

		/*HTML5重点功能*/
		HTML5: useHtml5, //可选 true,false  是否使用HTML5进行切图 ，支持HTML5浏览器会使用HTML5进行切图，没有HTML5浏览器则采用原始的方式(先上传大图再截取)，SHEARPHOTO这个方案无可挑剔了吧！
		...
		/*HTML5重点功能*/
		HTML5Effects: HTML5Effects, //是否开启图片特效功能给用户  可选true false,  提示：有HTML5浏览器才会开启的！当然开启HTML5切图，该设置才有效
		...
		url:cropImgUrl, //后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了

		scopeWidth: 600, //可拖动范围宽  也就是"main"对象的初始大小(整数型，禁止含小数点) 宽和高的值最好能一致  
		...
		Max: 800, //一开始启动时，图片的宽和高，有时候图片会很大的，必须要设置一下(整数型，禁止含小数点)，尽可能和scopeWidth值 一致  
	...
	Shear.complete = function(serverdata) { //截图成功完成时，由shearphoto.php返回数据过来的成功包
			...
			layui.use('layer', function(laytpl){ 
			var $=layui.jquery;
			var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
			var src=relativeUrl +serverdata[0]["ImgUrl"];
			  parent.layer.msg('您的切圖路徑是'+src);
			  parent.$('.upload-show-img-now').attr('src',src);
			  var oldPath=parent.$('.upload-show-input-now').val();
			  parent.$('.upload-show-input-now').val(src);
			  parent.$('.upload-show-input-now').next('div').children('.upload-check-img,.upload-delete-img').show();
			  if(oldPath){
				  $.post(deleteImgUrl,{'url':oldPath},function(data){
					  oldPath="";
					});
			  }
	...
	var up = new ShearPhoto.frameUpImg({
		...
		url: uploadImgUrl, //HTML5切图时，不会用到该文件，后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了
	...
	Shear.addEvent(document.getElementById("saveShear"), "click", function() { //按下截图事件，提交到后端的shearphoto.php接收
		Shear.SendPHP({
			shearphoto: document.getElementById("saveWidth").value,
			saveUrl: saveUrl
	...
	try {
		webcam.init_config = {
			...
			webcam.set_api_url(uploadImgUrl); //拍照连接后端的URL文件，后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了    
		webcam.set_swf_url(styleUrl + "images/webcam.swf"); //拍照是FLASH的，那么你懂的，已经在本版本中帮你加入相对路径，你基本不用改这里了
		webcam.set_shutter_sound(true, styleUrl + "images/shutter.mp3") //拍照声音，参数1 是否开启， 参数2：声音路径,设置错误，后果是拍照失败，已经在本版本中帮你加入相对路径，你基本不用改这里了
